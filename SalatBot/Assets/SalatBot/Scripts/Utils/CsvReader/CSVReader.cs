...
namespace SalatBot.Scripts.Utils.CsvReader
{
    ...
    {
        private static readonly string[] SPLIT_RE = new string[2]
        {
            @";(?=(?:[^""]*""[^""]*"")*(?![^""]*""))",
            @",(?=(?:[^""]*""[^""]*"")*(?![^""]*""))"
        };
        ...
        public static List<Dictionary<string, object>> Read(string file)
        {
            ...
            var header = Regex.Split(lines[0], string.Join("|", SPLIT_RE));
            for (var i = 1; i < lines.Length; i++)
            {
                var values = Regex.Split(lines[i], string.Join("|", SPLIT_RE));
